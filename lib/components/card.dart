...
class Card extends PositionComponent with DragCallbacks, TapCallbacks, HasWorldReference<KlondikeWorld> {
  ...
  void onDragEnd(DragEndEvent event) {
    ...
    final bool shortDrag = (position - _whereCardStarted).length < KlondikeGame.dragTolerance;
    ...
    final List<Pile> dropPiles = parent!.componentsAtPoint(position + size / 2).whereType<Pile>().toList();
    if (dropPiles.isNotEmpty) {
      if (dropPiles.first.canAcceptCard(this)) {
        ...
        if (dropPiles.first is TableauPile) {
          ...
          final NotifyingVector2 dropPosition = (dropPiles.first as FoundationPile).position;
          doMove(
            dropPosition,
            onComplete: () {
